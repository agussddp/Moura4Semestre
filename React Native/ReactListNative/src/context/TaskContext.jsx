...
export const TaskProvider = ({ children }) => {
  ...
  const getTasks = async () => {
    try {
      const APIReturn = await axios.get("http://172.16.2.126:3000/taskpoint");
  ...
  const postTasks = async (taskValue) => {
    try {
      // cadastro na API
      await axios.post("http://172.16.2.126:3000/taskpoint", {
        descricao: taskValue,
      });
  ...
  const deleteTask = async (id) => {
    try {
      await axios.delete(`http://172.16.2.126:3000/taskpoint/${id}`);
      getTasks();
    } catch (error) {
  ...
  const putTaskConfirm = async (tarefa) => {
    try {
      await axios.put(`http://172.16.2.126:3000/taskpoint/${tarefa.id}`, {
